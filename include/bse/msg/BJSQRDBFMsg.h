#ifndef BSE_MSG_B_J_S_Q_R_D_B_F_MSG_H
#define BSE_MSG_B_J_S_Q_R_D_B_F_MSG_H
...
class BJSBSDBFMsg;
// 非交易确认库
class BJSQRDBFMsg : public dbf::DBFRecord {
public:
  BJSQRDBFMsg();
  explicit BJSQRDBFMsg(const BJSBSDBFMsg &);
  ~BJSQRDBFMsg();
...
public:
  void setLSHM(const std::string &value) { lSHM_ = value; }
  const std::string &lSHM() const { return lSHM_; }
  ...
  void setCWDH(const std::string &value) { cWDH_ = value; }
  const std::string &cWDH() const { return cWDH_; }

  void setQSBZ(const std::string &value) { qSBZ_ = value; }
  const std::string &qSBZ() const { return qSBZ_; }
